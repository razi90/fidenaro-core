...
impl EnvironmentSpecifier for ScryptoTestEnvironmentSpecifier {
    ...
    // type DefiPlazaV2Pool = DefiPlazaV2PoolInterfaceScryptoTestStub;
    ...
    // type DefiPlazaV2Adapter = DefiPlazaV2Adapter;
...
impl ScryptoTestEnv {
    ...
    pub fn new_with_configuration(configuration: Configuration) -> Result<Self, RuntimeError> {
        ...
        let protocol_owner_badge = ResourceBuilder::new_fungible(OwnerRole::None)
            .divisibility(0)
            .mint_initial_supply(1, &mut env)?;
        ...
        let [ignition_package, simple_oracle_package, ociswap_v1_adapter_v1_package, caviarnine_v1_adapter_v1_package] =
            Self::PACKAGE_NAMES.map(|name| Self::publish_package(name, &mut env).unwrap());

        // Creating the various resources and their associated pools.
        let resource_addresses = Self::RESOURCE_DIVISIBILITIES.try_map(|divisibility| {
        ...
        let ociswap_v1_pools = resource_addresses.try_map(|resource_address| {
            let mut ociswap_pool = OciswapV1PoolInterfaceScryptoTestStub::instantiate(
                *resource_address,
                XRD,
                configuration.fees,
                FAUCET,
            ...
            let _ = ociswap_pool.add_liquidity(resource_x, resource_y, &mut env)?;
        ...
        let bin_span = 100;
        let caviarnine_v1_pools = resource_addresses.try_map(|resource_address| {
            let mut caviarnine_pool = CaviarnineV1PoolInterfaceScryptoTestStub::new(
                rule!(allow_all),
                rule!(allow_all),
                *resource_address,
                XRD,
            ...
            let resource_x =
                ResourceManager(*resource_address).mint_fungible(dec!(2_000_000_000), &mut env)?;
            let resource_y = ResourceManager(XRD).mint_fungible(dec!(2_000_000_000), &mut env)?;
        ...
        let (ociswap_v2_package, ociswap_v2_adapter_v1_package, ociswap_v2_pools) = {
            ...
            let registry = OciswapV2RegistryInterfaceScryptoTestStub::instantiate(
            ...
            let ociswap_v2_pools = resource_addresses.try_map(|resource_address| {
                ...
                let (mut ociswap_pool, ..) = OciswapV2PoolInterfaceScryptoTestStub::instantiate(
                    resource_x,
                    resource_y,
                    pdec!(1),
                    60,
                ...
                let _ = ociswap_pool
                    .add_liquidity(-10_000, 10_000, resource_x, resource_y, &mut env)?;
        ...
        let (defiplaza_v2_package, defiplaza_v2_adapter_v1_package, defiplaza_v2_pools) = {
            let defiplaza_v2_pool_package = {
                ...
                env.call_function_typed::<_, PackagePublishWasmOutput>(
                    ...
                    PACKAGE_PUBLISH_WASM_IDENT,
                    &PackagePublishWasmInput {
                        code: defiplaza_v2_package_wasm.to_vec(),
                        definition: defiplaza_v2_package_definition,
                        metadata: Default::default(),
                    },
                )?
                .0
    ...
    fn publish_package(
        ...
        Package::publish(code, definition, Default::default(), env).map(|item| item.0)
    }
}
