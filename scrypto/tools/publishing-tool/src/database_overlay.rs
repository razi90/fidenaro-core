...
// TODO: This implementation is copied from radixdlt-scrypto into this repo so
// that we don't have to upgrade the scrypto dependency of Ignition and the node
// to a new one (especially since a bunch of things have been moved). The commit
// hash that this was copied from is `30b0956f7ec394ac981a2580d9c927aedd6ffc25`.
// This is an exact copy with no changes. Once we update to a Scrypto dependency
// that includes this, we should remove this copied code.

#![allow(dead_code, clippy::mem_replace_with_default)]
...
pub type MergeableSubstateDatabaseOverlay<'a, S> =
    SubstateDatabaseOverlay<&'a mut S, S>;
pub type OwnedSubstateDatabaseOverlay<S> = SubstateDatabaseOverlay<S, S>;

pub struct SubstateDatabaseOverlay<S, D> {
    /// The database overlay. All commits made to the database are written to
    /// the overlay. This covers new values and deletions too.
    overlay: StagingDatabaseUpdates,

    /// A mutable or immutable reference to the root database that this type
    /// overlays. It only needs to be mutable if you wish to commit to the root
    /// store. To be useful, `S` should implement at least `Borrow<D>`.
    root: S,
...
impl<S: BorrowMut<D>, D: CommittableSubstateDatabase>
    SubstateDatabaseOverlay<S, D>
{
    pub fn commit_overlay_into_root_store(&mut self) {
        let overlay = core::mem::replace(
            &mut self.overlay,
            StagingDatabaseUpdates::default(),
        );
        self.get_writable_root().commit(&overlay.into());
    }
}

impl<S: Borrow<D>, D: SubstateDatabase> SubstateDatabase
    for SubstateDatabaseOverlay<S, D>
{
                ...
                Some(StagingNodeDatabaseUpdates { partition_updates }) => {
                    match partition_updates.get(partition_num) {
                        ...
                        Some(StagingPartitionDatabaseUpdates::Reset {
                            ...
                            // In a partition reset we delete all substates in a
                            // partition and can also write new substates there. If
                            // the substate that we're looking for can't be found in
                            // the new substate values of a partition delete then it
                            // is one of the deleted substates. Therefore, the
                            // following will report that it has found the substate
    ...
    fn list_entries_from(
        ...
        // This function iterates over entries of the specified partition.
        // Therefore, we don't need to think about other partitions
        // here. We first check if there are any partition updates
        // for the specified partition. If there is not, no overlaying is needed
        // and we can just return the iterator of the root store.
        let from_sort_key = from_sort_key.cloned();
        match self.overlay.node_updates.get(node_key) {
            // There is a partition update in the overlay.
            Some(StagingNodeDatabaseUpdates { partition_updates }) => {
                match partition_updates.get(partition_num) {
                    // The partition was reset. None of the substates of this
                    // partition that exist in the root
                    // store "exist" anymore. We just need an iterator over the
                    // new substates in the reset action.
                    Some(StagingPartitionDatabaseUpdates::Reset {
                        new_substate_values,
                    }) => {
                        match from_sort_key {
                            // A `from_sort_key` is specified. Only return sort
                            // keys that are larger than or equal to the from
                            // sort key. We do this through BTreeMap's range
                            // function instead of doing filtering. We're able
                            // to do this since a `BTreeMap`'s keys are always
                            // sorted.
                            Some(from_sort_key) => Box::new(
                                new_substate_values.range(from_sort_key..).map(
                            ...
                            // No `from_sort_key` is specified. Start iterating
                            // from the beginning.
                    ...
                    Some(StagingPartitionDatabaseUpdates::Delta {
                        ...
                        let underlying =
                            self.get_readable_root().list_entries_from(
                                partition_key,
                                from_sort_key.as_ref(),
                            );
                        ...
                        match from_sort_key {
                            ...
                            Some(from_sort_key) => {
                                ...
                                Box::new(OverlayingIterator::new(
                            ...
                            // from the beginning.
                            None => {
                                let overlaying = substate_updates.iter().map(
                                    |(sort_key, database_update)| {
                                        match database_update {
                                            DatabaseUpdate::Set(
                                                substate_value,
                                            ) => (
                    ...
                    // Overlay doesn't contain anything for the provided
                    // partition number. Return an iterator over the data in the
                    // root store.
                    None => self.get_readable_root().list_entries_from(
                        partition_key,
                        from_sort_key.as_ref(),
                    ),
                }
            }
            // Overlay doesn't contain anything for the provided node key.
            // Return an iterator over the data in the root store.
            None => self
                .get_readable_root()
    ...
    fn list_partition_keys(
        ...
        let overlying = self
            ...
            .flat_map(
                |(
                    node_key,
                    StagingNodeDatabaseUpdates { partition_updates },
                )| {
                    partition_updates.keys().map(|partition_num| {
                        DbPartitionKey {
        ...
        Box::new(
            OverlayingIterator::new(underlying, overlying)
                .map(|(value, _)| value),
        )
    }
}
...
fn merge_database_updates(
    this: &mut StagingDatabaseUpdates,
    other: DatabaseUpdates,
) {
    for (
        other_node_key,
        NodeDatabaseUpdates {
    ...
    ) in other.node_updates.into_iter()
    {
        ...
        match this.node_updates.get_mut(&other_node_key) {
            ...
            Some(StagingNodeDatabaseUpdates {
                ...
                {
                    // Check if the partition num exists in `this` database
                    // updates
                    match this_partition_updates.get_mut(&other_partition_num) {
                        // The partition exists in both `this` and `other` and
                        ...
                        Some(this_partition_database_updates) => {
                            match (
                                ...
                                // override anything in `this` with anything in
                                // `other`.
                                (
                                ...
                                // Whatever the current state is, if the other
                                // database update is a partition reset then it
                                // takes precedence.
                                (
                                    this_partition_database_updates,
                        ...
                        // The partition num does not exist in `this` database
                        // updates. This merge is simple, just insert it.
                        None => {
            ...
            // The node key does not exist in `this` database updates. This
            // merge is simple, just insert it.
            None => {
    ...
    fn from(value: DatabaseUpdates) -> Self {
        Self {
            node_updates: value
                ...
                .into_iter()
                .map(|(key, value)| {
                    (key, StagingNodeDatabaseUpdates::from(value))
                })
                .collect(),
        }
    }
...
struct StagingNodeDatabaseUpdates {
    partition_updates:
        BTreeMap<DbPartitionNum, StagingPartitionDatabaseUpdates>,
}
    ...
    fn from(value: StagingNodeDatabaseUpdates) -> Self {
        Self {
            partition_updates: value
                ...
                .into_iter()
                .map(|(key, value)| {
                    (key, PartitionDatabaseUpdates::from(value))
                })
                .collect(),
        }
    }
    ...
    fn from(value: NodeDatabaseUpdates) -> Self {
        Self {
            partition_updates: value
                ...
                .into_iter()
                .map(|(key, value)| {
                    (key, StagingPartitionDatabaseUpdates::from(value))
                })
                .collect(),
        }
    }
...
impl From<StagingPartitionDatabaseUpdates> for PartitionDatabaseUpdates {
    fn from(value: StagingPartitionDatabaseUpdates) -> Self {
        match value {
            StagingPartitionDatabaseUpdates::Delta { substate_updates } => {
                Self::Delta {
...
/// An iterator overlaying a "change on a value" (coming from the [`overlaying`]
/// iterator) over a "base value" (coming from the [`underlying`] iterator).
/// The one is matched to another by a `K` part (of the iterated tuple `(K,
/// V)`), which both iterators are assumed to be ordered by.
    ...
    O: Iterator<Item = (K, Option<V>)>,
{
    ...
    /// The [`overlaying`] one provides the "changes" to those values,
    /// represented as `Option<V>`:
    /// - A [`Some`] is an upsert, i.e. it may override an existing base value,
    ///   or "insert" a completely new one to the iterated results.
    /// - A [`None`] is a delete, which causes the base value to be omitted in
    ///   the iterated results.
            ...
            if let Some(overlaying_key) = self.overlaying.peek_key() {
                if let Some(underlying_key) = self.underlying.peek_key() {
                    match underlying_key.cmp(overlaying_key) {
                        Ordering::Less => {
                            return self.underlying.next(); // return and move it
                                                           // forward
                        }
                        Ordering::Equal => {
                ...
                let (overlaying_key, overlaying_change) =
                    self.overlaying.next().unwrap();
                match overlaying_change {
                    Some(value) => return Some((overlaying_key, value)),
                    None => continue, /* we may need to skip over an
                                       * unbounded number of deletes */
                }
            } else {
